...
$$
\text{DRAFT -- update: May 18th 2022}
$$
...
and sampling from the marginal posterior distribution
\begin{eqnarray*}
  \pi(\phi, \eta \mid y) & \propto & \pi(\phi, \eta) \pi(y \mid \phi, \eta) \\
    & \propto & \pi(\phi, \eta) \int \pi(y \mid \theta, \phi, \eta) \mathrm d \theta,
\end{eqnarray*}
using MCMC, or any other inference methods, on $\phi$ and $\eta$.
It is furthermore possible to recover samples for the posterior distribution of $\theta$,
via a two-steps sampling procedure
\begin{eqnarray*}
  \phi, \eta & \sim & \pi(\phi, \eta \mid y) \\
  \theta & \sim & \pi(\theta \mid \phi,\eta,  y).
\end{eqnarray*}
Together, these steps generate a sample from the joint posterior distribution $\pi(\phi, \eta, \theta \mid y)$.

We may also be interested in doing inference on a new set of latent variables, $\theta^*$, for which we have no observations -- or at the very least, these observations were not used to fit the model.
In a Machine Learning setting, $\theta$ may correspond to the latent variables for the training set, while $\theta^*$ would be the latent variable for a test set.
The model is augmented accordingly, with a prior covariance between $\theta$ and $\theta^*$,
$$
  (\theta, \theta^*) \sim \text{Normal}(0, {\bf K}(\phi)),
$$
where $\bf K$ is the total covariance matrix, and
$$
 {\bf K(\phi)} = \begin{bmatrix}
   K(\phi) & K^*(\phi) \\
   K^*(\phi) & K^{**} (\phi),
 \end{bmatrix}
$$
where $K$ is the prior covariance for $\theta$, $K^{**}$ the prior covariance for $\theta^*$ and $K^*$ the covariance between $\theta$ and $\theta^*$.

Posterior samples for $\theta^*$ can also be obtained in a post-sampling step by drawing from
$$
  \theta^* \sim \pi(\theta^* \mid y, \phi, \eta),
$$
where the conditional distribution on the RHS depends on $\bf K$.


Unfortunately, in all but a few simple cases
we cannot calculate $\pi(\phi \mid y)$, $\pi(\theta \mid \phi, y)$,
or $\pi(\theta^* \mid \phi, y)$ explicitly.
...
In the diagonal case, `hessian_block_size = 1` (default value).
